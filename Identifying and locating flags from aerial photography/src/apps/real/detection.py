...
MAVLINK_CONN = "serial:///COM8:115200"  # استخدم السيريل بورت
...
def mavsdk_thread(connection_url):
    async def run_mavsdk():
            ...
            async for pos in drone.telemetry.position():
                with telemetry.lock:
                    ...
                    if a is None or a == 0.0:
                        ...
                        print("telemetry.alt --------------->>>>>>>>>>>> " ,telemetry.alt)
                    else:
                        telemetry.alt = a
        ...
        tasks = [asyncio.create_task(attitude_task()), asyncio.create_task(position_task())]
        try:
            while running.is_set():
...
def processor_thread():
    try:
        model = YOLO(MODEL_PT)     
        model.to("cuda:0")            
        print("[Processor] Model loaded on GPU.")
    except Exception as e:
        print(f"[Processor] Error loading model '{MODEL_PT}': {e}")
        running.clear()
        return
    ...
    while running.is_set():
        frame = None
        with qCond:
            if not qCond.wait_for(lambda: len(frameQueue) > 0 or not running.is_set(), timeout=0.1):
                continue
            if len(frameQueue) > 0:
        ...
        try:
            results = model(frame, imgsz=640, conf=0.3 , iou=0.45)
        except Exception as e:
            print("[Processor] Inference error:", e)
            continue
            ...
            if boxes is not None and len(boxes) > 0:
                for box in boxes:
                    try:
                        ...
                        xyxy = np.array(box.xyxy).reshape(-1)[:4]
                        conf = float(box.conf)
                        cls = int(box.cls)
        ...
        for (left, top, width, height, conf, cls) in dets:
            cx = left + width // 2
            cy = top + height // 2
            ...
            cv2.putText(frame, txt, (left, max(30, top-6)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,255,255), 1)
            cv2.putText(frame, coord_txt, (left, min(frame_h-4, top+height+18)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,255,255), 1)
            print("->>>>>>>->>>>>>>->>>>>>>->>>>>>>->>>>>>>- [ ",coord_txt," ]")
...
if __name__ == "__main__":
    t_reader = threading.Thread(target=reader_thread, args=(VIDEO_PATH,), daemon=True)
