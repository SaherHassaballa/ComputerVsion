    ...
    while running.is_set():
        ...
        with qCond:
            if not qCond.wait_for(
                lambda: len(frameQueue) > 0 or not running.is_set(), timeout=0.1
            ):
                continue
            if len(frameQueue) > 0:
        ...
        for left, top, width, height, conf, cls in dets:
            ...
            if geo is not None:
                ...
                coord_txt = "Geo Fail"
            frameNum += 1
            frameName = r"C:\Users\saher\Desktop\Github Projects\computer vision\projects\Identifying and locating flags from aerial photography\src\data\prediction\frame\img{frameNum}"
            txt = f"cls:{cls} {conf:.2f}"
            cv2.rectangle(frame, (left, top), (left + width, top + height), color, 2)
            cv2.imwrite(frameName, frame)
            with open(coord_file, "a") as f:
                f.write(f"coord_txt :frame {frameNum} , {coord_txt}")
            cv2.putText(
                frame,
                txt,
                (left, max(30, top - 6)),
            ...
            cv2.putText(
                frame,
                coord_txt,
                (left, min(frame_h - 4, top + height + 18)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.45,
                (0, 255, 255),
                1,
